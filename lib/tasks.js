...
import * as vectorFunctions from './vector.js';
...
export async function update(taskId, options = {}, deps = {}) {
  ...
  // Fetch existing task to preserve fields TickTick resets when omitted
  const existing = await apiRequest('GET', `/task/${encodeURIComponent(resolvedTaskId)}`, undefined, deps);
  const input = { id: resolvedTaskId, title: existing.title };
